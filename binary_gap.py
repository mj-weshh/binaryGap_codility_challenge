...
def binaryGap(num):
     ...
     if len(index) == 1 or len(index) == 0:
          ...
          #if there are more than one gaps, the greatest difference is assigned to gap
          else:
               gap = max(differences)
...
def main():
     ...
     #parsing the binary code as a string to allow the len() in binaryGap()
     num_bin2 = str(num_bin2)

     #outputs the representation of the users input as a binary code
     print(f"The binary representation of {num} is: {num_bin2}")
     ...
     print(f"The binary gap of {num} is: {gap}")
